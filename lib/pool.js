...
var apiInterfacesFall = require('./apiInterfaces.js')(config.daemonfall, config.wallet);
...
function getBlockTemplateFall(callback){
    apiInterfacesFall.rpcDaemon('getblocktemplate', {reserve_size: 8, wallet_address: config.poolServer.poolAddress}, callback);
}
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
	    ...
	    //fallover
	    getBlockTemplateFall(function(errorF, resultF){
		if (loop)
		    setTimeout(function(){
			jobRefresh(true);
		    }, config.poolServer.blockRefreshInterval);
		if (errorF){
		    log('error', logSystem, 'FALLOVER Error polling getblocktemplate %j', [errorF);
		    callback(false);
		    return;
		}
		if (!currentBlockTemplate || resultF.height > currentBlockTemplate.height){
		    log('error', logSystem, 'FALLOVER New block to mine at height %d w/ difficulty of %d', [resultF.height, resultF.difficulty]);
		    processBlockTemplate(resultF);
		}
		callback(true);
	    })
        }
        if (!error && (!currentBlockTemplate || result.height > currentBlockTemplate.height)){
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
		...
		//fallover
		apiInterfacesFall.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(errorF, resultF){
		    if (errorF){
			log('error', logSystem, 'Fallover Error submitting block at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.login, miner.ip, shareType, errorF]);
			recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
		    }
		    else{
			...
			log('info', logSystem,
			    ...
			    [blockFastHash.substr(0, 6), job.height, miner.login, miner.ip, resultF]
			);
